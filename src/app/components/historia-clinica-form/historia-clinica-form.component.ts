...
export class HistoriaClinicaFormComponent implements AfterContentInit {
  ...
  async ngOnInit() {
    ...
    if (this.usuario.data.perfil === 'Paciente') {
      ...
      let historiasPaciente = this.historiasClinicas.filter(
        (historia: any) => historia.data.paciente.id === this.usuario.id
      );
      let aux: any[] = [];
      ...
      if (this.usuario.data.perfil === 'Especialista') {
        ...
        historiasPacientes = this.historiasClinicas.filter(
          (historia: any) => historia.data.especialista.id === this.usuario.id
        );
      } else {
        this.tableDetalle = [
          'fecha',
  ...
  actualizarTabla() {
    if (this.form.valid) {
      this.lista.data = this.historiasClinicas.filter(
        (historia: any) =>
          historia.data.paciente.id === this.usuario.id &&
          historia.data.especialista.id === this.form.value.especialista.id
      );
    }
  }
...
Deben ser botones rectangulares con el nombre de la especialidad debajo.LISTO
...
Estos botones deben ser rectangulares. Formato (09 de Septiembre).LISTO
...
especialista haya atendido al menos 1 vez."	Además, Mostrar los usuarios con un favbutton redondo , imagen y nombre, al seleccionar un paciente  se muestra los dellaes de los turnos y un acceso a la reseña de cada consulta, 
LISTO
---------------------
