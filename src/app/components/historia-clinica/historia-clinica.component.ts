  ...
  async ngOnInit() {
    if (!this.esFiltro) {
      ...
      this.historiaBuscada = this.historiasClinicas.filter((historia: any) => {
        ...
        return (
          historia.data.paciente.id === this.data.turno.pacienteObj.id &&
          historia.data.turno.horario === this.data.turno.horario &&
          this.formatearFecha(historia.data.turno.fecha.toDate()) ===
            this.formatearFecha(aux)
        );
      })[0];
      if (this.historiaBuscada) {
        ...
        this.form
          .get('temperatura')
          ?.setValue(this.historiaBuscada.data.temperatura);
        ...
        if (this.historiaBuscada.data.claveUno) {
          ...
          this.form
            .get('claveUno')
            ?.setValue(this.historiaBuscada.data.claveUno);
          this.form
            .get('valorUno')
            ?.setValue(this.historiaBuscada.data.valorUno);
        }
        if (this.historiaBuscada.data.claveDos) {
          this.indice++;
          this.form
            .get('claveDos')
            ?.setValue(this.historiaBuscada.data.claveDos);
          this.form
            .get('valorDos')
            ?.setValue(this.historiaBuscada.data.valorDos);
        }
        if (this.historiaBuscada.data.claveTres) {
          this.indice++;
          this.form
            .get('claveTres')
            ?.setValue(this.historiaBuscada.data.claveTres);
          this.form
            .get('valorTres')
            ?.setValue(this.historiaBuscada.data.valorTres);
        }
      }
    } else {
      this.indice = 1;
