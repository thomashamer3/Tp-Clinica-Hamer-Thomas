...
export class EncuestaComponent {
  ...
  encuesta: any;
  pregunta1 =
    '¿Cómo calificaría su experiencia general con la clínica en línea?';
  pregunta2 =
    '¿Qué tan satisfecho/a está con la facilidad de acceso a los servicios de la clínica online?';
  pregunta3 =
    '¿La plataforma de la clínica cumplió con sus expectativas en términos de usabilidad?';
  pregunta4 = '¿Recomendaría esta clínica en línea a otros pacientes?';
  pregunta5 =
    'En una escala del 1 al 10, ¿cómo calificaría su experiencia general con la clínica en línea?';
  pregunta6 =
    '¿Tiene algún comentario adicional sobre su experiencia con el especialista ?';
    ...
    for (let i = 0; i < this.turnos.length; i++) {
      ...
      if (this.data.datos.turnoObj.id === turno.id) {
        for (let j = 0; j < turno.data.dia.length; j++) {
          ...
          if (
            this.sonFechasIguales(new Date(dia.fecha), this.data.datos.fecha)
          ) {
            this.diaEncuesta = j;
    ...
    if (this.encuesta !== null && this.esEncuesta) {
      ...
      const html = `
        <div  style="text-align: left">
          ...
          <h5>Respuesta:<strong>${
            this.respuestas[0][this.encuesta.pregunta1]
          }</strong></h5>
          <h5>2.${this.pregunta2}</h5>
          <h5>Respuesta:<strong>${
            this.respuestas[0][this.encuesta.pregunta2]
          }</strong></h5>
          <h5>3.${this.pregunta3}</h5>
          <h5>Respuesta:<strong>${
            this.respuestas[1][this.encuesta.pregunta3]
          }</strong></h5>
          <h5>4.${this.pregunta4}</h5>
          <h5>Respuesta:<strong>${
            this.respuestas[2][this.encuesta.pregunta4]
          }</strong></h5>
        </div>
        `;
  ...
  async enviarExperiencia() {
    this.formEspe
      .get('calificacion')
      ?.setValue(this.slider.nativeElement.value);

    if (this.formEspe.valid) {
      let aux: any = {};

      this.data.datos.turnoObj.data.dia[this.diaEncuesta].hora[
        this.horaEncuesta
      ].encuestaExperiencia = this.formEspe.value;
