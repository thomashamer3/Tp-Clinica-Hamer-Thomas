...
export class EspecialidadesComponent {
  ...
  async agregarEspecialidad() {
    let formValido =
      this.form.valid && (this.imagenes ? this.imagenes.length : 0) === 1;
    this.cargando = true;
    if (formValido) {
      let foto = await this.storage.guardarFoto(
        this.imagenes[0],
        'especialidades'
      );
      await this.firestore.guardar(
        {
          nombre: this.form.value.especialidad,
          estaMarcado: false,
          foto: foto,
        },
        'especialidades'
