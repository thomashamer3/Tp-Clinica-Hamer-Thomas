...
export class FirebaseAuthService {
        ...
        if (this.firebaseAuth.currentUser)
          await sendEmailVerification(this.firebaseAuth.currentUser)
            .then((resultado) => {
              Swal.fire(
                'OK',
                'Mail de verificacion enviado, verifiquelo para ingresar',
                'success'
              );
            })
            .catch((error) => {
  ...
  async obtetenerUsuarioLogueadoBase(uid: any) {
    ...
    usuarios.forEach((element: any) => {
      let credencialesBase = JSON.parse(
        element.data.credenciales ? element.data.credenciales : '{}'
      );
  ...
  async login({ email, password }: any) {
    return signInWithEmailAndPassword(this.firebaseAuth, email, password).catch(
      (error) => {
